...
const PLACEHOLDERS = [
  { id: "btn-placeholder", key: "bw", label: "Black & white" },
  { id: "btn-mosaic", key: "mosaic", label: "Mosaic" },
  { id: "btn-blush", key: "blush", label: "Blush" },
];
...
let placeholder = null; // "bw" | "mosaic" | "blush" — keyless stand-in style
...
function applyMode() {
  ...
  placeholderGroup.classList.toggle("hidden", haveInside);
}
...
insideClear.addEventListener("click", (e) => {
  ...
  const ready = haveAI || placeholder;
...
// Keyless stand-in styles, drawn from the main video itself.
function setPlaceholder(key) {
  placeholder = key;
  for (const p of PLACEHOLDERS)
    document.getElementById(p.id).setAttribute("aria-pressed", p.key === key);
}

for (const p of PLACEHOLDERS) {
  document.getElementById(p.id).addEventListener("click", () => {
    if (!videoFile) return;
    setPlaceholder(p.key);
    haveAI = false;
    btnPlay.disabled = false;
    btnExport.disabled = false;
    status(`${p.label} style active — preview or export, no key needed.`);
  });
}
setPlaceholder(null);
...
// Offscreen buffer for the mosaic: the frame is drawn tiny, then blown back
// up with smoothing off, which is what makes the chunky blocks.
const tiny = document.createElement("canvas");
const tinyCtx = tiny.getContext("2d");
const MOSAIC_BLOCKS = 22; // blocks across the frame's long edge

function drawMosaic() {
  const long = Math.max(canvas.width, canvas.height);
  tiny.width = Math.max(2, Math.round((canvas.width / long) * MOSAIC_BLOCKS));
  tiny.height = Math.max(2, Math.round((canvas.height / long) * MOSAIC_BLOCKS));
  tinyCtx.drawImage(orig, 0, 0, tiny.width, tiny.height);
...
const PLACEHOLDER_FILTERS = {
  bw: "grayscale(1) contrast(1.2) brightness(1.04)",
  blush: "grayscale(1) sepia(1) hue-rotate(278deg) saturate(2.2) brightness(1.1)",
};
...
function drawWindow(q, presence, vid) {
  ...
  if (vid) {
    ...
    ctx.filter = PLACEHOLDER_FILTERS[placeholder] || PLACEHOLDER_FILTERS.bw;
    ctx.drawImage(orig, 0, 0, canvas.width, canvas.height);
    ctx.filter = "none";
  }
  ctx.restore();
